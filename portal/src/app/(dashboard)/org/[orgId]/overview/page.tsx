...
import {
  Activity, Zap, AlertTriangle, CheckCircle2, Clock,
  TrendingUp, Server, Cpu, Database, Wifi, ArrowUpRight,
} from 'lucide-react';
...
function KpiCard({ label, value, sub, icon: Icon, color, trend }: {
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl font-bold text-white count-up" style={{ fontFamily: 'Space Grotesk' }}>
        {value}
      </p>
    ...
    <div className="space-y-1">
      {events.map((e, i) => (
        <motion.div
          ...
          className="flex items-start gap-3 py-1.5 px-2 rounded-lg hover:bg-white/3 transition-all"
        >
          <span className="text-base shrink-0">{agentEmoji(e.agentId)}</span>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-medium text-white" style={{ color: agentColor(e.agentId) }}>
                {e.agentName}
              </span>
...
export default function OverviewPage() {
  ...
  // Fetch real server uptime from Ghost heartbeat (public endpoint)
  useEffect(() => {
    async function fetchUptime() {
      ...
      } catch { /* Ghost may be offline — keep '--:--:--' */ }
    }
    fetchUptime();
  ...
  // Tick the displayed uptime every second after we have the base value
  useEffect(() => {
  ...
  return (
    <div className="p-6 max-w-screen-2xl mx-auto">
      ...
      >
        <KpiCard label="Agents Online" value={onlineCount} sub={`of ${agentList.length} total`}
                 icon={Zap} color="#00D4FF" trend="+0%" />
        <KpiCard label="Active Now"    value={workingCount} sub="processing tasks"
                 icon={Activity} color="#F59E0B" />
        <KpiCard label="Errors"        value={errorCount}   sub="requires attention"
                 icon={AlertTriangle} color={errorCount > 0 ? '#EF4444' : '#10B981'} />
        <KpiCard label="Server Uptime" value={uptime}      sub="Ghost gateway"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="glass rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {[
                ...
                { label: 'Ollama (qwen3)',   status: true,         detail: 'CPU inference' },
          ...
          {/* Powered by */}
          <div className="glass rounded-2xl p-5">
            <h3 className="text-xs font-semibold text-ghost-muted mb-3 uppercase tracking-wider">Stack</h3>
            <div className="grid grid-cols-2 gap-2">
              {[
                { label: 'qwen3:8b',     color: '#27AE60', desc: 'Local LLM' },
                { label: 'Grok 3 Fast',  color: '#1DA1F2', desc: 'Research' },
                { label: 'GPT-4o',       color: '#10B981', desc: 'Vision' },
                { label: 'Claude',       color: '#7C3AED', desc: 'Synthesis' },
                { label: 'Pinecone',     color: '#00D4FF', desc: 'Memory' },
                { label: 'Neon PG',      color: '#3B82F6', desc: 'Database' },
              ].map((s) => (
                <div key={s.label} className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg"
                  ...
                  <div>
                    ...
                    <p className="text-[9px] text-ghost-muted/60">{s.desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between text-[10px] text-ghost-muted/30 font-mono">
        ...
        <span>Created by Terry · v2.0 · {new Date().getFullYear()}</span>
      </div>
    </div>
  );
}
