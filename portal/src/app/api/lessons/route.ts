...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${OPENCLAW_API}/api/lessons${qs ? `?${qs}` : ''}`, {
      headers: headers(),
    ...
    return NextResponse.json({ lessons: [], total: 0, error: 'Ghost offline' });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${OPENCLAW_API}/api/lessons`, {
      method:  'POST',
      headers: headers(),
