...
export async function GET(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${OPENCLAW_API}/api/tasks${qs ? `?${qs}` : ''}`, {
      headers: headers(),
    ...
    return NextResponse.json({ tasks: [], total: 0, error: 'Ghost offline' });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const res = await fetch(`${OPENCLAW_API}/api/tasks`, {
      method:  'POST',
      headers: headers(),
